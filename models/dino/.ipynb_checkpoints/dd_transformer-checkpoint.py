...
from .ops.modules import MSDDAttn
...
class DDTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if deformable_encoder:
            encoder_layer = DDTransformerEncoderLayer(d_model, dim_feedforward, kernel_size, dilation,
                                                          dropout, activation,
        ...
        if deformable_decoder:
            decoder_layer = DDTransformerDecoderLayer(d_model, dim_feedforward, kernel_size, dilation,
                                                          dropout, activation,
...
class DDTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024, kernel_size=3, dilation=[1, 2, 3],
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=1,
                 add_channel_attention=False,
                 use_deformable_box_attn=False,
                 box_attn_type='roi_align',
                 ):
        ...
        if use_deformable_box_attn:
            ...
            self.self_attn = MSDDAttn(d_model, n_levels, n_heads, n_points, kernel_size=kernel_size, dilation=dilation)
...
class DDTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, kernel_size=3, dilation=[1, 2, 3],
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=1,
                 use_deformable_box_attn=False,
                 box_attn_type='roi_align',
        ...
        if use_deformable_box_attn:
            ...
            self.cross_attn = MSDDAttn(d_model, n_levels, n_heads, n_points, kernel_size=kernel_size, dilation=dilation)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
